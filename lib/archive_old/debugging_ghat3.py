...
#User-defined inputs
P=2**12
M = int(P/1)  # sequence length (non-zero values)
Delta_t = tau_k[0]/10.0  # timestep
...
ini_par = {
    ...
    'arms': 3,
    'Ge': 0.0,
    'G_i': [1.0e6,1.0e5,1.0e4],  # initial guesses for G1 to G3
...
ax.plot(omega,gnu_ctft_fit.real, color='red',lw=3, label='g_nu_ctft_fit')
...
Ge_fit, tau_fit, G_fit= nls_fit(omega,g_nu, **ini_par)

dtft_orig = dtft_gt_finite(omega, Delta_t, G_k, tau_k, G_e, M)

#Ge_fit, tau_fit, G_fit= nls_fit(omega[1::],dtft_orig, **ini_par)

dtft_fit = dtft_gt_finite(omega, Delta_t, G_fit, tau_fit, Ge_fit, M)

dtft2 = dtft_gt_finite(omega, Delta_t, G_k, tau_k, G_e, M)

fig, ax = plt.subplots(figsize=(8,6))
ax.plot(omega,g_nu.real, color='orange',lw=8, label='g_nu_fft')
ax.plot(omega,dtft_orig.real, color='cyan',lw=5, label='g_rel_dtft_an')
ax.plot(omega,dtft_fit.real, color='red',lw=3, label='g_rel_dtft_fit')
