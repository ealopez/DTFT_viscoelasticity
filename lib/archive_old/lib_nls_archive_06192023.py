...
def Y(omega_range, delta_t, G, tau, M,Ge=0):
    
    
    return np.array( [ Ge*delta_t/(1.0*np.exp(-1.0j*omega_range[i]*delta_t) ) + sum(G[:]*delta_t/(1.0 - np.exp(-delta_t/tau[:])*np.exp(-1.0j*omega_range[i]*delta_t)) ) for i in range(len(omega_range)) ] )



def X(omega, delta_t, G, tau, M, Ge=0.0):
    """
    function that returns the DTFT of the a) aliased version of the relaxation modulus, b) the aliased and spectrum leaked version of the relaxatiuon modulus

    Parameters
    ----------
    omega : np.array of floats
        array of radian frequencies to evaluate the DTFT (ranging from zero to 2pi)
    delta_t : float
        timestep between acquired samples. The inverse of the sampling frequency
    G : np.array of floats
        DESCRIPTION.
    tau : np.array of floats
        DESCRIPTION.
    M : int
        length of experimental/simulated stress relaxation trace
    Ge : float, optional
        equilibrium modulus, stress relaxation response at long times, default is zero

    Returns
    -------
    aliased : np. array of floats
        DTFT of the aliased version of the relaxation modulus
    aliased_leaked: np.array of floats
        DTFT of the aliased and spectrum leaked version of the relaxation modulus

    """
    k=len(G)
    first_term = sum([G[i] / (1 - np.exp(-delta_t / tau[i]) * np.exp(-1j * omega * delta_t)) for i in range(k)])
    second_term = sum([G[i] * np.exp(-M * delta_t / tau[i]) * np.exp(-1j * M * omega * delta_t) / (1 - np.exp(-delta_t / tau[i]) * np.exp(-1j * omega * delta_t)) for i in range(k)])
    third_term = Ge / (1 - np.exp(-1j * omega * delta_t))
    fourth_term = Ge * np.exp(-1j * M * omega * delta_t) / (1 - np.exp(-1j * omega * delta_t))
    aliased = ( first_term+third_term)*delta_t
    aliased_leaked = (first_term - second_term + third_term - fourth_term)*delta_t
    return aliased, aliased_leaked 
...
def dtft_gt_finite(omega, Delta_t, G_k, tau_k, G_e, M):
    """
    ...
    term2 = np.sum ([G_k[k] * (1.0 - np.exp(-M * Delta_t / tau_k[k]) * np.exp(-1.0j *M* omega * Delta_t)) / (1 - np.exp(-Delta_t / tau_k[k]) * np.exp(-1.0j * omega * Delta_t)) for k in range(len(G_k))],axis=0 )
...
def calculate_residual2(model, G_hat):
    """
    ...
    if np.iscomplexobj(G_hat):  # If G_hat is complex
        denominator_real = (G_hat.real)
        denominator_imag = (G_hat.imag)
        mask_real = denominator_real > 0
        mask_imag = denominator_imag > 0
        residual_real = np.zeros_like(model.real)
        residual_imag = np.zeros_like(model.imag)
        residual_real[mask_real] = np.abs ( (model.real[mask_real] - G_hat.real[mask_real]) / denominator_real[mask_real] )
        residual_imag[mask_imag] = np.abs( (model.imag[mask_imag] - G_hat.imag[mask_imag]) / denominator_imag[mask_imag] )
        residual = (residual_real + residual_imag) / 2.0
        residual = np.abs(np.abs(model)-np.abs(G_hat))
        ...
        residual[mask] = np.abs((model[mask] - G_hat[mask]) / G_hat[mask])  # Compute residual where G_hat is not zero
